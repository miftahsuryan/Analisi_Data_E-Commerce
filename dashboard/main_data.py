...
sales_df = pd.read_csv('combined_sales_kategori_df.csv')
city_df = pd.read_csv('order_per_city_df.csv')
state_df = pd.read_csv('order_per_state_df.csv')
...
#pemesan per city
    ...
    with tab1:
        ...
        st.dataframe(city_df.head(number_of_rows))
    ...
    with tab2:
        ...
        kota = st.selectbox("Pilih kota:", city_df['customer_city'].unique())
        
        #filter
        filtered_city = city_df[city_df['customer_city'] == kota]
        ...
        ax.set_title(f'Total Order untuk {kota}')
        
        st.pyplot(fig)
        st.metric(
            label=f"Jumlah Order di {kota}", 
            value=f"{filtered_city[city_count_col].sum()}",
            delta=f"{filtered_city[city_count_col].sum() / city_df[city_count_col].sum() * 100:.1f}% dari total"
        )
    ...
    with tab3:
        ...
        #top 10 cities
        top_cities = city_df.sort_values(by=city_count_col, ascending=False).head(10)
        ...
        ax.set_ylabel('Kota')
        ax.set_title('10 Kota dengan Jumlah Order Tertinggi')
        ax.invert_yaxis()  
...
# pemesanan per state
    ...
    with tab1:
        ...
        st.dataframe(state_df.head(number_of_rows))
    ...
    with tab2:
        ...
        state = st.selectbox("Pilih negara bagian:", state_df['customer_state'].unique())
        
        #filter
        filtered_state = state_df[state_df['customer_state'] == state]
        ...
        if chart_type == "Bar Chart":
            ...
            ax.set_ylabel('Jumlah Order')
            ax.set_title(f'Total Order untuk {state}')
        else:
            #menggunakan visualisasi data pie chart
            fig, ax = plt.subplots(figsize=(10, 6))
            top_states = state_df.sort_values(by=state_count_col, ascending=False).head(5)
            others = pd.DataFrame({
                'customer_state': ['Others'],
                state_count_col: [state_df[state_count_col].sum() - top_states[state_count_col].sum()]
            })
            ...
            ax.axis('equal')
            ax.set_title(f'Distribusi Order per Negara Bagian (Highlight: {state})')
            
        st.pyplot(fig)
        st.metric(
            label=f"Jumlah Order di {state}", 
            value=f"{filtered_state[state_count_col].sum()}",
            delta=f"{filtered_state[state_count_col].sum() / state_df[state_count_col].sum() * 100:.1f}% dari total"
        )
    ...
    with tab3:
        ...
        #top 10 states
        top_states = state_df.sort_values(by=state_count_col, ascending=False).head(10)
        ...
        ax.set_title('10 Negara Bagian dengan Jumlah Order Tertinggi')
        ax.invert_yaxis()  
